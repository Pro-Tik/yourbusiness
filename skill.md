...
- **What it does:** Pulls exactly 5 pending leads from the \`campaign_leads\` SQLite database.
- **Output:** Returns data in clean JSON format for you to read.

**Tool 2: \`node fire_whatsapp.js <phone>\`**
- **What it does:** Sends the dynamically drafted message via the Evolution API to the provided target phone number. The script explicitly fetches the name and area from the database internally to prevent hallucinations.
...
2. For each lead, execute \`node fire_whatsapp.js <phone>\` to send the "3,500 BDT Promo" message. Wait for the script to finish (including its 60-second delay).
